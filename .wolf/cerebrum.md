...
- [2026-07-15] Example commissioning UX is now a single Material Stepper. Runtime commissioning state lives in `example/lib/commissioning_controller.dart`; UI entrypoint is `example/lib/commissioning_page.dart`; `home_page.dart` FAB should route there instead of the deleted onboarding/wifi/provisioning pages.
- [2026-07-16] Matter simulator tooling is now `tools/matter_sim`, an installable Python CLI (`matter-sim`) that replaces the old WSL2 shell script. It orchestrates connectedhomeip setup/build/run, on-network and BLE simulator launch, factory reset, and default onboarding code checks.
