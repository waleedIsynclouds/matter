...
class WifiInputPage extends StatefulWidget {
  ...
  final String code;
  const WifiInputPage(
      {super.key, required this.onboardingPayload, required this.code});
      ...
      body: Padding(
        ...
        child: Column(children: [
          TextField(
            ...
            decoration: InputDecoration(hintText: 'WifiName'),
          ),
          TextField(
            controller: _passwordController,
            decoration: InputDecoration(
                hintText: 'WifiPassword (optional for open networks)'),
          ),
          SizedBox(
            height: 16,
          ),
          ElevatedButton(
              onPressed: () {
                ...
                if (ssid.trim().isEmpty) {
                  Fluttertoast.showToast(msg: '请输入wifi名称');
                  return;
                }
                SharedPreferences.getInstance().then((sp) {
